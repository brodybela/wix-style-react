import {createDriverFactory} from '../test-common';
import statsWidgetDriverFactory from './StatsWidget.driver';
import StatsWidget from './StatsWidget';
import {isEnzymeTestkitExists, isTestkitExists} from '../../testkit/test-common';
import {statsWidgetTestkitFactory} from '../../testkit';
import {statsWidgetTestkitFactory as enzymeStatsWidgetTestkitFactory} from '../../testkit/enzyme';

describe('StatsWidget', () => {
  const createDriver = createDriverFactory(statsWidgetDriverFactory);

  const title = 'Stats Widget title';
  const statistics = [{
    title: '10$',
    subtitle: 'Revenue'
  },
  {
    title: '2',
    subtitle: 'Products'
  },
  {
    title: '1',
    subtitle: 'Transactions'
  },
  {
    title: '5',
    subtitle: 'Profit'
  },
  {
    title: '15',
    subtitle: 'Music'
  }];

  const statisticsWithPersents = [{
    title: '10$',
    subtitle: 'Revenue',
    persent: 15
  },
  {
    title: '2',
    subtitle: 'Products',
    persent: -15
  },
  {
    title: '1',
    subtitle: 'Transactions',
    persent: 0
  }];

  const dropdownOption = [
    {id: 0, value: 'This month'},
    {id: 1, value: 'This week'}];

  let driver;

  const stub = console.error = jest.fn();

  function createComponent(props) {
    driver = createDriver(<StatsWidget {...props}/>).driver;
  }

  let React;

  beforeEach(() => {
    React = require('react');
  });

  afterEach(() => {
    jest.resetModules();
    stub.mockReset();
  });

  it('should have correct title', () => {
    createComponent({title, statistics});
    expect(driver.titleText()).toBe(title);
  });

  it('should show statistics', () => {
    createComponent({title, statistics});
    expect(driver.getStatisticTitle(0)).toBe(statistics[0].title);
    expect(driver.getStatisticSubTitle(0)).toBe(statistics[0].subtitle);
  });

  it('should show abs of persentage', () => {
    createComponent({title, statistics: statisticsWithPersents});
    expect(driver.getStatisticPersentValue(0)).toBe(Math.abs(statisticsWithPersents[0].persent) + '%');
    expect(driver.getStatisticPersentValue(1)).toBe(Math.abs(statisticsWithPersents[1].persent) + '%');
  });

  it('should put proper classes to persentage according to value', () => {
    createComponent({title, statistics: statisticsWithPersents});

    expect(driver.getStatisticPersentClass(0)).toContain('isPositive');
    expect(driver.getStatisticPersentClass(1)).toContain('isNegative');

    expect(driver.getStatisticPersentClass(2)).not.toContain('isNegative');
    expect(driver.getStatisticPersentClass(2)).not.toContain('isPositive');
  });

  it('should show filters', () => {
    const stub = jest.fn();
    const children = <StatsWidget.Filter selectedId={1} options={dropdownOption} dataHook="stats-widget-filter" onSelect={stub}/>;
    createComponent({title, statistics, children});
    driver.selectFilter('stats-widget-filter');
    expect(stub).toHaveBeenCalled();
  });

  it('should not initialize component with 1 bad child', () => {
    const PageRequiredChildrenArrayError = 'Warning: Failed prop type: Invalid prop `children` of type `object` supplied to `StatsWidget`, expected an array.\n    in StatsWidget';
    createComponent({title, statistics, children: <div/>});

    expect(stub).toHaveBeenCalledWith(PageRequiredChildrenArrayError);
  });

  it('should not initialize component with persent which are not a numbers', () => {

    const wrongStatistics = [{
      title: '10$',
      subtitle: 'Revenue',
      persent: '15%'
    },
    {
      title: '2',
      subtitle: 'Products',
      persent: '-15%'
    },
    {
      title: '1',
      subtitle: 'Transactions',
      persent: '0'
    }];

    const PageRequiredChildrenArrayError = 'Warning: Failed prop type: Invalid prop `statistics[0].persent` of type `string` supplied to `StatsWidget`, expected `number`.\n    in StatsWidget';
    createComponent({title, statistics: wrongStatistics});

    expect(stub).toHaveBeenCalledWith(PageRequiredChildrenArrayError);
  });

  describe('testkit', () => {
    it('should exist', () => {
      expect(isTestkitExists(<StatsWidget title="test title" statistics={statistics}/>, statsWidgetTestkitFactory)).toBeTruthy();
    });
  });

  describe('enzyme testkit', () => {
    it('should exist', () => {
      expect(isEnzymeTestkitExists(<StatsWidget title="test title" statistics={statistics}/>, enzymeStatsWidgetTestkitFactory)).toBeTruthy();
    });
  });
});
